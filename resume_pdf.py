...
def _register_fonts():
    ...
    pdfmetrics.registerFont(
        TTFont("Calibri", os.path.join(FONT_DIR, "Carlito-Regular.ttf"))
    )
    pdfmetrics.registerFont(
        TTFont("Calibri-Bold", os.path.join(FONT_DIR, "Carlito-Bold.ttf"))
    )
    pdfmetrics.registerFont(
        TTFont("Calibri-Italic", os.path.join(FONT_DIR, "Carlito-Italic.ttf"))
    )
    pdfmetrics.registerFont(
        TTFont("Calibri-BoldItalic", os.path.join(FONT_DIR, "Carlito-BoldItalic.ttf"))
    )
...
def _styles():
    return {
        ...
        "bullet": ParagraphStyle(
            ...
            textColor=GRAY,
            leading=SIZE["bullet"] * 1.25,
        ),
        "job_title": ParagraphStyle(
            "job_title",
            fontName=FONT_BOLD,
            fontSize=SIZE["job_title"],
            leading=SIZE["job_title"] * 1.15,
            textColor=NAVY,
        ),
        "job_dates": ParagraphStyle(
            "job_dates",
            fontName=FONT_BOLD,
            fontSize=SIZE["job_dates"],
            leading=SIZE["job_dates"] * 1.15,
            textColor=GRAY,
            alignment=2,
        ),
...
def _link_run(text, url):
    return f'<link href="{url}"><font color="{GRAY_HEX}"><u>{text}</u></font></link>'
...
def build_resume_pdf(data, output_path=None):
    ...
    story.append(
        Paragraph(
            ...
            f"{_link_run(data['email'], 'mailto:' + data['email'])}  |  "
            + "  |  ".join(_link_run(l["text"], l["url"]) for l in data["links"]),
            styles["contact"],
        )
    )
    ...
    for s in data["skills"]:
        story.append(
            Paragraph(
                f'<b><font color="{NAVY_HEX}">{s["label"]}:</font></b> {s["value"]}',
                styles["body"],
            )
        )
    ...
    for job in data["experience"]:
        row = Table(
            [
                [
                    ...
                    Paragraph(job["dates"], styles["job_dates"]),
                ]
            ],
        ...
        bullets = [
            ListItem(Paragraph(b, styles["bullet"]), spaceAfter=1)
        ...
        story.append(
            ListFlowable(
                ...
                leftIndent=12,
                bulletFontName=FONT,
                bulletFontSize=SIZE["bullet"],
            )
        )

    story += _section_heading("Key Projects", styles)
    for p in data["projects"]:
        title_html = f'<b><font color="{NAVY_HEX}">{p["title"]}</font></b>'
        if p.get("link"):
            title_html += f" ({_link_run(_link_label(p['link']), p['link'])})"
        story.append(
            Paragraph(
                f"{title_html}<br/>&bull; {p['description']} "
                f'<font color="{GRAY_HEX}"><i>Stack: {p["stack"]}</i></font>',
                styles["body"],
            )
        )
